...
# 30-min timestamps from 05:00 to 15:00 (21 revisions)
times_30min = []
for h in range(5, 16):
    for m in [0, 30]:
        if h == 15 and m > 0:
...
print("=== Starting Remaining MP State Plants 30-Min Revisions (05:00 to 15:00) ===")
print(f"Date: {DATE}")
print(f"Plants: {[p[0] for p in REMAINING_MP_PLANTS]}")
print(f"Total Revisions per Site: {len(times_30min)}")
print(f"Revisions: {times_30min}")
...
print("\n=======================================================")
print(f"REMAINING MP PLANTS 30-MIN REVISIONS COMPLETE")
print("=======================================================")
df_res = pd.DataFrame(all_records)
print(df_res.to_string(index=False))
df_res.to_csv("remaining_mp_plants_30min_summary.csv", index=False)
