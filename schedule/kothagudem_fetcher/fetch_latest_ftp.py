"""Fetch the newest file from an FTP server and save it locally for Kothagudem.

Defaults are chosen for a Kothagudem deployment, but all settings can be
...
DEFAULT_LOCAL_DIR = Path("kothagudem_fetcher") / "downloads"
DEFAULT_FILENAME_PREFIX = "kothagudem_"
...
def download_latest_file(config: FTPConfig, local_dir: Path | str | None = None) -> tuple[Path, str, str]:
    if not config.password:
        raise SystemExit("FTP password not provided. Set SFTP_PASSWORD or pass --password.")

    target_dir = Path(local_dir or config.local_dir)
