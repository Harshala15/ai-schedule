...
SITES = [
    ('BALAKWADA', 'BALAKWADA-ai-intellis-scheduler'),
    ('BAMKHAL', 'BAMKHAL-ai-intellis-scheduler'),
    ('ANJANGOAN', 'ANJANGOAN-ai-intellis-scheduler')
]

# Generate 30-min timestamps from 05:00 to 15:00 (21 revisions)
times_30min = []
for h in range(5, 16):
    for m in [0, 30]:
        if h == 15 and m > 0:
            break
        times_30min.append(f"{h:02d}:{m:02d}")

print("=== Starting BALAKWADA, BAMKHAL, ANJANGAON 30-Min Revisions (05:00 to 15:00) ===")
print(f"Date: {DATE}")
print(f"Total Revisions per Site: {len(times_30min)}")
print(f"Revision Timestamps: {times_30min}")
...
for site_id, fn_name in SITES:
    ...
    for idx, r_time in enumerate(times_30min, 1):
        slug = r_time.replace(':', '-')
        payload = {
            "site_id": site_id,
            "target_date": DATE,
        ...
        print(f"[{idx}/{len(times_30min)}] Invoking {site_id} at {r_time}...")
        success = False
        max_attempts = 4
...
print("\n=======================================================")
print(f"BALAKWADA, BAMKHAL, ANJANGAON 30-MIN REVISIONS COMPLETE")
print("=======================================================")
df_res = pd.DataFrame(all_results)
print(df_res.to_string(index=False))
df_res.to_csv("balakwada_bamkhal_anjangaon_30min_summary.csv", index=False)
