"""Configuration for the Kothagudem forecast scheduler package."""
...
# Kothagudem runs a 3-hour rolling horizon from each revision time.
FORECAST_HORIZON_HOURS = _env_int("KOTHAGUDEM_FORECAST_HORIZON_HOURS", 3)
FORECAST_BLOCKS = _env_int(
    "KOTHAGUDEM_FORECAST_BLOCKS",
    max(1, FORECAST_HORIZON_HOURS * 60 // config.BLOCK_MINUTES),
)

# S3 layout used by the scheduler package.
DEFAULT_S3_RAW_OWNER = "vedanjay"
DEFAULT_S3_PLANT_FOLDER = "KOTHAGUDEM"
...
ECMWF_TILT_DEGREES = _env_int("KOTHAGUDEM_ECMWF_TILT_DEGREES", int(round(config.PLANT_TILT_DEG)))
ECMWF_AZIMUTH_DEGREES = _env_int(
    "KOTHAGUDEM_ECMWF_AZIMUTH_DEGREES",
    int(round(config.to_openmeteo_azimuth(config.PLANT_ORIENTATION_FROM_SOUTH_DEG))),
)
