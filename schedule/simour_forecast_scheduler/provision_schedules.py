"""Create or update the daily EventBridge Scheduler rules for SIMOUR."""
...
def _schedule_name(time_str: str) -> str:
    return f"simour-forecast-{time_str.replace(':', '')}"
...
def upsert_schedules() -> list[dict]:
    lambda_arn = _require_env("SIMOUR_FORECAST_LAMBDA_ARN")
    role_arn = _require_env("SIMOUR_EVENTBRIDGE_INVOKE_ROLE_ARN")
    ...
    for time_str in CAPTURE_TIMES:
        ...
        kwargs = dict(
            ...
            State="ENABLED",
            Description=f"SIMOUR forecast run for {time_str}",
        )
        try:
